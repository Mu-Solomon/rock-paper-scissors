import { useState } from "react";

export default function Picked(selections) {
  return (
    <div className="flex  w-full  mt-24 sm:mt-20 mb-20 sm:mb-[-50px] gap-5 sm:gap-20 bg-no-repeat  bg-center flex-wrap justify-between items-center sm:justify-center ">
      <div className="h-50 sm:h-110 sm:justify-center  sm:flex-col-reverse  flex flex-col items-center  justify-center sm:flex-wrap-reverse ">
        <div className="h-36 sm:h-90  flex flex-col ">
          <div
            style={{ backgroundColor: selections.selected.user.wrapperColour }}
            className=" w-32 h-[134px] sm:w-68 sm:h-71 rounded-full"
          >
            <div
              style={{
                backgroundColor: selections.selected.user.mainColour,
              }}
              className="w-32 h-32 sm:h-68 sm:w-68  flex justify-center items-center rounded-full mx-auto"
            >
              <div className="bg-[#d8d5d5] rounded-full w-22 h-[93px]  sm:w-50 sm:h-[212px] flex items-end ">
                <div className="bg-white rounded-full w-22 h-22 sm:h-50 sm:w-50 p-6 mx-auto ">
                  <img
                    src={selections.selected.user.imageSource}
                    className="mx-auto sm:w-28 sm:mt-6"
                    alt="Rock Icon"
                  />
                </div>{" "}
              </div>
            </div>{" "}
          </div>
        </div>
        <p className="text-center text-white font-semibold sm:text-2xl sm:pt-1 sm:py-7 tracking-widest">
          YOU PICKED
        </p>
      </div>

      {selections.selected.host === null ? (
        <div className="flex  flex-col sm:flex-col-reverse h-50 sm:h-110 sm:justify-around ">
          <div className="flex flex-col items-center justify-center h-40 sm:h-48 ">
            <div className="bg-[#14163946] rounded-full w-28 h-28 sm:w-48 sm:h-48 p-6 mx-auto sm:mt-[-180px]"></div>
          </div>
          <p className="text-white font-semibold  sm:text-2xl sm:py-7 mt-[-75px] tracking-widest">
            THE HOUSE PICKED
          </p>
        </div>
      ) : (
        <div className="h-50 sm:h-110 sm:justify-center  sm:flex-col-reverse  flex flex-col items-center  justify-center sm:flex-wrap-reverse ">
          <div className="h-36 sm:h-90  flex flex-col ">
            <div
              style={{
                backgroundColor: selections.selected.host.wrapperColour,
              }}
              className=" w-32 h-[134px] sm:w-68 sm:h-71 rounded-full"
            >
              <div
                style={{
                  backgroundColor: selections.selected.host.mainColour,
                }}
                className="w-32 h-32 sm:h-68 sm:w-68  flex justify-center items-center rounded-full mx-auto"
              >
                <div className="bg-[#d8d5d5] rounded-full w-22 h-[93px]  sm:w-50 sm:h-[212px] flex items-end ">
                  <div className="bg-white rounded-full w-22 h-22 sm:h-50 sm:w-50 p-6 mx-auto ">
                    <img
                      src={selections.selected.host.imageSource}
                      className="mx-auto sm:w-28 sm:mt-6"
                      alt="Rock Icon"
                    />
                  </div>{" "}
                </div>
              </div>{" "}
            </div>
          </div>
          <p className="text-center text-white font-semibold sm:text-2xl sm:pt-1 sm:py-7 tracking-widest">
            THE HOUSE PICKED
          </p>
        </div>
      )}
    </div>
  );
}
